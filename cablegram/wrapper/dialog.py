...
def download_dialogs(client, offset):
    try:
        dialogs = client.send(functions.messages.GetDialogs(offset_date=0, offset_id=0, offset_peer=types.InputPeerEmpty(), limit=0))
...
def fetch_dialogs(client):
        return download_dialogs(client, 0)